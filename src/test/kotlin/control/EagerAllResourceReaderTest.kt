...
class EagerAllResourceReaderTest {
    ...
    fun weapons() {
        ...
        sut.weapons()[4] shouldBeEqualToComparingFields Weapon(
            name = "Bolt Rifle",
            keywords = listOf(WeaponKeyword.ASSAULT),
            damage = "1",
            shots = BigDecimal("2"),
            strength = 4,
            armourPiercing = 1,
        ...
        sut.weapons()[9] shouldBeEqualToComparingFields Weapon(
        ...
        sut.weapons()[10] shouldBeEqualToComparingFields Weapon(
    ...
    fun targets() {
        ...
        sut.targets()[3] shouldBeEqualToComparingFields Model(
            ...
            wounds = BigDecimal(5),
            frequency = "0.1",
            weapons = mapOf(Pair(sut.weapons()[0], 1))
        )
    }

    @Test
    fun units() {
        sut.units().size shouldBe 2
        //Intercessor Squad
        sut.units()[0] shouldBeEqualToComparingFields CombatUnit(
            name = "Intercessor Squad",
            additionalName = null,
            models = mapOf(Pair(sut.targets()[1], 5)),
            totalPoints = 95
        )
        //Desolation Squad
        sut.units()[1] shouldBeEqualToComparingFields CombatUnit(
            ...
            additionalName = null,
            models = mapOf(Pair(sut.targets()[2], 5)),
            totalPoints = 170
        )
    }
}